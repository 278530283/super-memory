...
class ReviewStrategyService {
   ...
   * 根据策略ID获取复习策略
   */
  async getReviewStrategyById(
    strategyId: string,
  ): Promise<ReviewStrategy | null> {
    try {
      ...
      console.error(
        "ReviewStrategyService.getReviewStrategyById error:",
        error,
      );
      throw error;
    }
  ...
  getReviewStrategyId(
    ...
    if (
      (proficiencyLevel === 0 && !isLongDifficult) ||
  ...
  restoreCardFromJSON(cardJSON: string): Card {
    try {
      ...
      console.warn(
        "[ReviewStrategyService] Failed to restore card from JSON, using empty card:",
        error,
      );
  ...
  async saveReviewScheduleLog(log: ReviewScheduleLog): Promise<void> {
    try {
      ...
      console.error(
        "ReviewStrategyService.saveReviewScheduleLog error:",
        error,
      );
      // 不抛出错误，避免影响主流程
    }
  ...
  async calculateReviewProgress(
    ...
    const reviewDate = DateUtils.formatToChinaDate(reviewTimeISO);
    console.log(
      "[ReviewStrategyService] calculateReviewProgress:",
      userWordProgress,
      proficiencyLevel,
      reviewDate,
    );
    ...
    // 检查是否已经复习过
    if (
      userWordProgress.last_review_date &&
      userWordProgress.last_review_date === reviewDate
    ) {
      console.log(
        "[ReviewStrategyService] Already reviewed today, no need to update.",
      );
      return null;
    }
    ...
    const isFirstReview =
      userWordProgress.strategy_id == null ||
      userWordProgress.start_date == null;
      ...
      if (userWordProgress.strategy_id !== STRATEGY_IDS.FSRS_DEFAULT) {
        ...
        const dayCount = Math.floor(
          (new Date(reviewDate).getTime() -
            new Date(userWordProgress.start_date!).getTime()) /
            (1000 * 60 * 60 * 24),
        );

        if (
          proficiencyLevel === 0 &&
          userWordProgress.proficiency_level === 3 &&
          dayCount >= 90
        ) {
          // 策略降级
          console.log(
            "[ReviewStrategyService] Strategy downgrade:",
            userWordProgress,
          );
    ...
    console.log(
      "[ReviewStrategyService] Updated user word progress:",
      updatedProgress,
    );
  ...
  async calculateNextReviewDate(
    ...
    if (strategyId === STRATEGY_IDS.FSRS_DEFAULT) {
      // FSRS策略
      const result = await this.calculateNextReviewDateFSRS(
        reviewTime,
        reviewedTimes,
        userWordProgress,
        proficiencyLevel,
      ...
      // 传统策略
      const result = await this.calculateNextReviewDateTraditional(
        strategyId,
        reviewTime,
        reviewedTimes,
        userWordProgress,
  ...
  async calculateNextReviewDateTraditional(
    ...
    try {
      ...
      console.error(
        "ReviewStrategyService.calculateNextReviewDateTraditional error:",
        error,
      );
      return {
        nextReviewDate: defaultDate,
  ...
  async calculateNextReviewDateFSRS(
    ...
    try {
      ...
      const scheduleDays = Math.ceil(
        (scheduledCard.card.due.getTime() - now.getTime()) /
          (1000 * 60 * 60 * 24),
      );
      ...
      return {
        nextReviewDate: DateUtils.formatToChinaDate(scheduledCard.card.due),
        fsrsCard: this.serializeCardToJSON(scheduledCard.card),
        reviewLog,
      };
    } catch (error) {
      console.error(
        "ReviewStrategyService.calculateNextReviewDateFSRS error:",
        error,
      );
  ...
  getFSRSPreviewOptions(
    ...
    try {
      ...
      console.error(
        "ReviewStrategyService.getFSRSPreviewOptions error:",
        error,
      );
      return [];
    }
  ...
  getDefaultNextReviewDate(reviewDate: string): string {
    ...
    return DateUtils.formatToChinaDate(nextReviewDate);
  }
}
