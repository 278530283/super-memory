...
export default function TestScreen() {
  ...
  const [currentTestActivityType, setCurrentTestActivityType] =
    useState<TestActivity | null>(null);
  ...
  useEffect(() => {
    ...
    console.log(
      "[TestScreen] currentTestActivity effect triggered. stateValue:",
      stateValue,
    );
    ...
    if (typeof stateValue === "string" && stateValue.startsWith("flow")) {
      ...
      if (match) {
        ...
        if (
          ["transEn", "transCh", "spelling", "pronunce", "listen"].includes(
            activityName,
          )
        ) {
          ...
          console.warn(
            `[TestScreen] Unrecognized test activity from state: ${activityName}`,
          );
        }
      } else {
        console.warn(
          `[TestScreen] Could not match activity from state: ${stateValue}`,
        );
      }
    }
  ...
  const CurrentTestComponent = useMemo<React.ComponentType<any> | null>(() => {
    console.log(
      "[TestScreen] CurrentTestComponent computed. currentTestActivityType:",
      currentTestActivityType,
    );
    return currentTestActivityType
      ? testComponentMap[currentTestActivityType]
      : null;
  }, [currentTestActivityType]);
  ...
  useEffect(() => {
    console.log(
      "[TestScreen] useEffect triggered. sessionId:",
      sessionId,
      "type:",
      type,
    );
  ...
  useEffect(() => {
    if (isTestFinished && wordIds.length > 0) {
      ...
      if (sessionId) {
        ...
        const progressField =
          type === "pre_test" ? "pre_test_progress" : "post_test_progress";
  ...
  const handleAnswer = useCallback(
    ...
    [storeHandleAnswer, currentWord, setStoreError, loadNextWord],
  );
  ...
  const handlePause = useCallback(() => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
    Alert.alert("暂停测试", "您确定要暂停测试吗？您的进度将会保存。", [
      {
        text: "取消",
        style: "cancel",
      },
  ...
  if (
    ...
    console.log(
      "[TestScreen] Render check - wordIds.length:",
      wordIds.length,
      "session:",
      !!session,
      "currentWord:",
    ...
    if (!storeIsLoading && !isTestFinished) {
      console.log(
        "[TestScreen] Render check - sessionId:",
        sessionId,
        "type:",
        type,
        "isLoading:",
